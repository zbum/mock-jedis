...
public class MockStorage {
    ...
    public synchronized long lrem(DataContainer key, long count, DataContainer value) {
        List<DataContainer> list = getListFromStorage(key, true);
        if (list == null) {
            throw new JedisDataException("ERR no data in " + key.getString());
        }
        ...
        if (count < 0) {
            ...
            for (int inx = size - 1; inx >= 0; inx--) {
                if (Objects.equals(list.get(inx), value)) {
                    list.remove(inx);
                    removedElements++;
                    if (removedElements == count) break;
                }
            }
        } else {
            int size = list.size();
